        ...
        public override bool Interact(Interactor interactor, Vector2i interactAt)
        {
            ...
            if (inventory.Contains(KnownItems.DIRTY_HANDS))
                options.Add(new Choice.Option(
                            "Wash hands",
                            new CustomAction(interactor, (Interactor _) => inventory.Remove(KnownItems.DIRTY_HANDS, -1))
                            ));
                ...
                options.Add(
                    new Choice.Option(
                        ...
                        new CustomAction(interactor, (Interactor _) =>
                        {
                            ...
                            {
                                inventory.Remove(dish.item, -1);
                                inventory.AddItem(
                                    new Item(dish.item.displayName, dish.item.traits & ~(Traits.Consumed | Traits.Invisible)),
                                    dish.count
                                    );
                            }
                        })
